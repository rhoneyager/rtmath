...
#include "../Ryan_Debug/Public_Domain/MurmurHash3.h"
#if USE_RYAN_SERIALIZATION
#include "../Ryan_Debug/Serialization/serialization_macros.h"
...
namespace {
	...
	std::multimap<size_t, Ryan_Debug::pHashStore> stores;
	...
	void loadStores()
	{
		...
		BOOST_LOG_SEV(lg, Ryan_Debug::debug::normal) << "Loading hash stores\n";
...

		{
			...
			if (!chash) {
				BOOST_LOG_SEV(lg, Ryan_Debug::debug::critical) << "Ryan_Debug configuration file does "
				"not have a /ddscat/hashes or /hashes key. Unable to load hash stores.\n";
				RDthrow(Ryan_Debug::debug::xMissingKey())
				<< Ryan_Debug::debug::otherErrorText("/ddscat/hashes in Ryan_Debug config is missing");
				return;
			}
			...
			for (const auto &c : children)
			{
				...
				BOOST_LOG_SEV(lg, Ryan_Debug::debug::debug_1) << "Parsed hash store:\n"
					<< "enabled: " << enabled
					<< "\npriority: " << priority
					<< "\npath: " << location
					<< "\ntype: " << type
				...
				std::shared_ptr < Ryan_Debug::hashStore > h;

				if (type == "dir") {
					h = std::shared_ptr<Ryan_Debug::hashStore>(new Ryan_Debug::hashStore);
				} /// \todo Add hash store plugin search code here, and fill in the store generator in the header file.
				else RDthrow(::Ryan_Debug::debug::xUnknownFileFormat())
					<< ::Ryan_Debug::debug::otherErrorText("Hash store code currently "
					"only supports \"dir\"-type stores. TODO: Add "
					"hash store plugin search code in hash.cpp and "
					"the header file.")
					<< ::Ryan_Debug::debug::hashType(type);

				h->writable = writable;
				h->base = boost::filesystem::path(location);
				h->type = type;

				Ryan_Debug::hashStore::addHashStore(h, priority);
			}
		}
	}
}
...
namespace Ryan_Debug {
	namespace registry {
		template class usesDLLregistry <
			::Ryan_Debug::hash_provider_registry,
			::Ryan_Debug::Hash_registry_provider >;
	}
	...
	*/
		...
		std::shared_ptr<registry::IOhandler> &sh, std::shared_ptr<registry::IO_options> &opts) const
	{
		...
		try {
			...
			BOOST_LOG_SEV(lg, Ryan_Debug::debug::warning) << serr.str();
		} catch (boost::filesystem::filesystem_error &f) {
			BOOST_LOG_SEV(lg, Ryan_Debug::debug::warning) 
				<< f.what() << " when searching " << base << " - " << hash << std::endl;
		} catch (boost::system::system_error &f) {
			BOOST_LOG_SEV(lg, Ryan_Debug::debug::error)
				<< f.what() << " when searching " << base << " - " << hash << std::endl;
			throw;
		} catch (...) {
			BOOST_LOG_SEV(lg, Ryan_Debug::debug::error) << "An unhandled error occurred "
	...
	HASH_t HASHfile(const std::string& filename)
	{
		...
		if (!serialization::detect_compressed(filename, cmeth, fname))
			RDthrow(::Ryan_Debug::debug::xMissingFile())
...
std::ostream & operator<<(std::ostream &stream, const Ryan_Debug::UINT128 &ob)
{
	stream << ob.string();
	return stream;
}
