...
#define DOTYPES(f) f(int); f(float); f(double); f(long); f(long long); \
	f(unsigned int); f(unsigned long); f(unsigned long long);
			...
			std::set<T> &expanded)
		{
			...
			if (interval < 0) {
				RDthrow(error::xInvalidRange())
					<< error::split_range_name(srange)
					<< error::specializer_type(specializer);
			}
			...
			if (specializer == "")
			{
				...
				if (start == 0 || end == 0)
					RDthrow(error::xDivByZero()) << error::split_range_name(srange)
						<< error::specializer_type(specializer);
...
#define SPEC_SPLITSET_A(T) \
	template void RYAN_DEBUG_DLEXPORT splitSet<T>(const T&, const T&, \
...
#define SPEC_SPLITSET_INTERVAL(T) \
	template void RYAN_DEBUG_DLEXPORT extractInterval<T>( \
...
#define SPEC_SPLITSET(T) \
	template void RYAN_DEBUG_DLEXPORT splitSet<T>(const std::string &instr, std::set<T> &expanded, \
	const std::map<std::string, std::string> *aliases);
		...
		void RYAN_DEBUG_DLEXPORT splitVector(
		...
		void RYAN_DEBUG_DLEXPORT splitNullMap(
...
#define IMPL_INTS(T) template class RYAN_DEBUG_DLEXPORT intervals < T >;
		DOTYPES(IMPL_INTS);

		/*
		template class RYAN_DEBUG_DLEXPORT intervals < int >;
		template class RYAN_DEBUG_DLEXPORT intervals < double >;
		template class RYAN_DEBUG_DLEXPORT intervals < float >;
		template class RYAN_DEBUG_DLEXPORT intervals < unsigned int >;
		template class RYAN_DEBUG_DLEXPORT intervals < long long >;
		template class RYAN_DEBUG_DLEXPORT intervals < long >;
		template class RYAN_DEBUG_DLEXPORT intervals < unsigned long >;
		template class RYAN_DEBUG_DLEXPORT intervals < unsigned long long >;
		*/
	}
}
