...
/// remove the Ryan_Debug dependency.
...
namespace Ryan_Debug
{
	namespace serialization
	{
		...
		void RYAN_DEBUG_DLEXPORT disable_auto_compression(bool);

		/// \brief Is auto-compression enabled or disabled?
		bool RYAN_DEBUG_DLEXPORT auto_compression_enabled();
		...
		**/
		template <typename T>
		bool RYAN_DEBUG_DLEXPORT detect_compressed(const T &base, std::string &meth, T &target);

		/// Convenience shortcut to compressed file detection.
		template <typename T>
		bool RYAN_DEBUG_DLEXPORT detect_compressed(const T &base);
		...
		**/
		template <typename T>
		bool RYAN_DEBUG_DLEXPORT detect_compression(const T &name, std::string &meth);
		...
		**/
		template <typename T>
		bool RYAN_DEBUG_DLEXPORT uncompressed_name(const T &name,
			T &uncompressed,
			std::string &meth);
		...
		**/
		template <typename T>
		bool RYAN_DEBUG_DLEXPORT select_compression(const T &filename, std::string &meth);
		...
		**/
		template <typename T>
		serialization_method RYAN_DEBUG_DLEXPORT select_format(const T &filename, T &outname);
		...
		template <typename T>
		bool RYAN_DEBUG_DLEXPORT known_format(const T &filename);

		/// Check to see if this is a serialization-known format. Works for either extensions of whole filenames.
		template<typename T>
		bool RYAN_DEBUG_DLEXPORT known_format(const T &filename, const serialization_method &meth);
		...
		**/
		void RYAN_DEBUG_DLEXPORT known_formats(std::string &output, bool includeCompressions = false);
		...
		**/
		void RYAN_DEBUG_DLEXPORT known_compressions(std::string &output, const std::string &base = "");
		...
		**/
		void RYAN_DEBUG_DLEXPORT prep_decompression(const std::string &meth,
		...
		**/
		void RYAN_DEBUG_DLEXPORT prep_compression(const std::string &meth,
		...
		**/
		...
		template<> void RYAN_DEBUG_DLEXPORT read<std::string>
		...
		void read(T &obj, boost::iostreams::filtering_istream &sin)
		{
			...
			//obj = in.str();
			Ryan_Debug::serialization::read<T>(obj, in);
			//in >> obj;
		}
		...
		**/
		...
		void read(T &obj, const std::string &infile)
		{
			...
			Ryan_Debug::serialization::read<T>(obj, sin);
		}
