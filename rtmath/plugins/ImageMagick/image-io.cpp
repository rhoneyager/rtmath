	...
	namespace registry
	{
			...
			const boost::shared_ptr<const rtmath::images::image > im)
		{
			...
			if (im->imageMaps.count(imkey)) eim = im->imageMaps.at(imkey);
				...
				<< Ryan_Debug::error::file_name(imkey);

			rtmath::plugins::ImageMagick::writeImage(filename, eim);
		...
		std::shared_ptr<IOhandler>
			...
			std::shared_ptr<const Ryan_Debug::registry::collectionTyped<rtmath::images::image> >)
		{
			using namespace rtmath::plugins::ImageMagick;
			std::string filename = opts->filename();
			...
			boost::shared_ptr<const Eigen::MatrixXf> eim = rtmath::plugins::ImageMagick::readImage(filename);
