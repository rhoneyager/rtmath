	...
	namespace plugins {
		namespace hdf5 {
				...
				boost::shared_ptr<rtmath::ddscat::ddPar > r)
			{
				...
				std::vector<Ryan_Debug::hash::HASH_t> vdielhashes((size_t)dielhashes.rows());
...
#undef near // No idea where this gets defined...
				...
				size_t wN;
				Ryan_Debug::splitSet::extractInterval(sWaves, wMin, wMax, wJunk, wN, sSpecialized);
				...
				*/
				...
				size_t bN, tN, pN;
				Ryan_Debug::splitSet::extractInterval(betas, bMin, bMax, junk, bN, sjunk);
				Ryan_Debug::splitSet::extractInterval(thetas, tMin, tMax, junk, tN, sjunk);
				Ryan_Debug::splitSet::extractInterval(phis, pMin, pMax, junk, pN, sjunk);
				...
				if ((size_t)planes.rows() != numPlanesCheck)
					RDthrow(Ryan_Debug::error::xAssert())
					<< Ryan_Debug::error::otherErrorText("Bad number of planes check when reading hdf5 ddpar");
...
namespace Ryan_Debug {
	namespace registry
	{
			...
			std::shared_ptr<const Ryan_Debug::registry::collectionTyped<rtmath::ddscat::ddPar> > filter)
		{
			...
			if (!grpShape) RDthrow(Ryan_Debug::error::xMissingHash())
