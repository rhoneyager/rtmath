...
namespace rtmath {
	...
	namespace plugins {
		/** \brief Namespace for the psql file readers.
		*
		* This plugin will probably only be used to read psql files. I already have HDF5 
		* write support and thus have no reason to write psql files. The plugin will 
		...
		**/
		namespace psql {
			...
			struct psql_handle : public rtmath::registry::IOhandler
			{
				psql_handle(const char* filename, IOtype t);
				virtual ~psql_handle();
			...
			Eigen::Matrix<DataType, Eigen::Dynamic, Eigen::Dynamic>
				getMatrix(const char* name, std::shared_ptr<psql_handle> h)
			{
					int status = 0;
					int parentId = h->file;
					int varid = 0;
