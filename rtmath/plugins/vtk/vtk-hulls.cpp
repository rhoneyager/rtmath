...
namespace rtmath
{
	namespace plugins
	{
		namespace vtk
		{
			class DLEXPORT_rtmath_voronoi vtkConvexHull : virtual public rtmath::ddscat::convexHull
			{
			protected:
				...
				vtkConvexHull(boost::shared_ptr< const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> > backend) : rtmath::ddscat::convexHull(backend)
				{
			...
			public:
				...
				virtual double volume() const { return _p->volume; }
				virtual double surfaceArea() const { return _p->surfarea; }
				virtual double maxDiameter() const { return _p->diameter; }
				virtual void principalAxes(double &beta, double &theta, double &phi) const { beta = _p->beta; theta = _p->theta; phi = _p->phi; }
				virtual void area2d(double out[3]) const { std::copy_n(_p->area2d, 3, out); }
				virtual void perimeter2d(double out[3]) const { std::copy_n(_p->perimeter2d, 3, out); }
			};
