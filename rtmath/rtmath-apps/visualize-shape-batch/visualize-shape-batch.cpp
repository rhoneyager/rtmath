...
int main(int argc, char** argv)
{
	...
	{
		using namespace std;
		cerr << "rtmath-visualize-shape-batch\n\n";
		...
		typedef std::pair<boost::shared_ptr<shapefile::shapefile>, boost::shared_ptr<Voronoi::VoronoiDiagram> > pairedData;
		vector<pairedData> voros;
		for (const auto &shp : shapes)
		{
			...
			voros.push_back(pairedData(shp,vd));
		}
		for (auto it = vdinputs.begin(); it != vdinputs.end(); ++it)
		{
			...
			try {
				// Handle not needed as the read context is used only once.
				vector<boost::shared_ptr<Voronoi::VoronoiDiagram> > vv;
				if (VoronoiDiagram::canReadMulti(nullptr, iopts))
					VoronoiDiagram::readVector(nullptr, iopts, vv);
				else {
				...
				for (const auto &loaded : vv)
				{
					boost::shared_ptr<shapefile::shapefile> shp = shapefile::shapefile::loadHash(loaded->hash());
					voros.push_back(pairedData(shp, loaded));
				}
			}
			catch (std::exception &e)
			{
		...
		{
			cerr << "Processing hash " << vd.first->hash().lower << endl;

			string filename("silos/");
			filename.append(vd.first->hash().string());
			filename.append(".silo");

			auto opts = registry::IO_options::generate();
			opts->filename(filename);

			handle = vd.first->writeMulti(handle, opts);
			...
			auto pic = [&](float nx, float ny, float nz, float ux, float uy, float uz, const std::string &suffix)
			{
				...
				ofile << "SaveWindowAtts = SaveWindowAttributes()\n"
					...
					"SaveWindowAtts.fileName = \"" << vd.first->hash().string() << suffix << "\"\n"
			...
			pic(0, 1, 0, 0, 0, -1, "-2");
			pic(0, 0, 1, 0, 1, 0, "-3");
			pic(0.651427, 0.531756, 0.541182, -0.240183, 0.82114, -0.517727, "-4");
