/// This program attempts to find the best possible effective density relation,
/// as a function of effective radius, maximum dimension and aspect ratio. This
/// involves reading known DDA results (part of the input file), and minimizing
/// the sum of the square of the differences. The norm can either be uniformly
/// weighted or can be biased according to a PSD.
...
int main(int argc, char *argv[])
{
	...
	try {
		cerr << "rtmath-oneellip-solver-basic" << endl;
		...
		runmatch.add_options()
			...
			("col-hash", po::value<int>()->default_value(1),
			 "Specify hash column")
			("col-dda", po::value<int>()->default_value(-1),
			 "Specify column that has DDA results")
		...
		refract.add_options()
			...
			;
		basic.add_options()
			("freq,f", po::value<std::string>(), "Specify frequency in GHz. Needed for dielectrics.")
			...
			("weighting", po::value<string>()->default_value("unweighted"),
			 "Specify weighting factors for the optimization. Can either be 'unweighted', "
			 "'ss', or 'TODO'")
			;
