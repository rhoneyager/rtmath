...
int main(int argc, char** argv)
{
	...
	try {
		...
		Eigen::VectorXf binTemps = Eigen::VectorXf::LinSpaced(maxObs.temp - minObs.temp + 1, minObs.temp, maxObs.temp);
		Eigen::VectorXf binWbTemps = Eigen::VectorXf::LinSpaced(maxObs.wbTemp - minObs.wbTemp + 1, minObs.wbTemp, maxObs.wbTemp);
		Eigen::VectorXf binSkinTemps = Eigen::VectorXf::LinSpaced(maxObs.skinTemp - minObs.skinTemp + 1, minObs.skinTemp, maxObs.skinTemp);
		const char *dimLabels[] = { "Temperature", "Wet bulb temperature", "Skin temperature" };
		const char *dimUnits[] = { "C", "C", "K" };
		...
		Eigen::Array3i span, maxs, mins;// = maxs - mins + 1;
		span(0) = maxObs.temp - minObs.temp + 1;
		span(1) = maxObs.wbTemp - minObs.wbTemp + 1;
		span(2) = maxObs.skinTemp - minObs.skinTemp + 1;
