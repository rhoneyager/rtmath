...
C   ANY DAMAGES THAT MAY RESULT FROM THE USE OF THE PROGRAM. 
      ...
      subroutine tmatrix_sub(rat,ndistr,axmax,npnax,b,gam,nkmax,
     &                       eps,np,lam,mrr,mri,ddelt,npna,ndgs,
     &                       cextin,cscat)
...
c      PRINT 7200, DDELT
 ...
 7400 FORMAT('LAM=',F10.6,3X,'MRR=',D10.4,3X,'MRI=',D10.4)
      ...
      DO 600 IAX=1,NPNAX
         ...
         R1 = 0.9999999*axi ! 0.89031D0*AXI
         R2 = 1.0000001*axi ! 1.56538D0*AXI
...
c         IF (DABS(RAT-1D0).LE.1D-6) PRINT 8003, REFF,VEFF
c         IF (DABS(RAT-1D0).GT.1D-6) PRINT 8004, REFF,VEFF
...
C           PRINT 6070, I,NMAX,NMAX1,NGAUSS
 6070       FORMAT(4I6)
  ...
  510    CONTINUE
         ...
         ASYMM=ALPH1(2)/3D0
c         PRINT 9100,CEXTIN,CSCAT,WALB,ASYMM
...
c         IF(KONTR.EQ.2) PRINT 3000,LL
         ...
         DO 50 I=1,11
            ...
            IF((DL+C2)*(DL-C3)-C1.LE.-1D-4) KONTR=2
            IF((DL-C2)*(DL+C3)-C1.LE.-1D-4) KONTR=2
c            IF(KONTR.EQ.2) PRINT 4000,LL,C
   50    CONTINUE
  100 CONTINUE
...
C        F12=-F12/F11
C        F34=F34/F11
c         PRINT 1004,TB,F11,F22,F33,F44,F12,F34
  500 CONTINUE
c      PRINT 1000 
