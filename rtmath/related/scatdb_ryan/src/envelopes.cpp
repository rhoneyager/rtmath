...
int main(int argc, char** argv) {
	try {
		...
		auto makeHeader = [&]() {
			...
			for (int i=0; i<db::data_entries::NUM_DATA_ENTRIES_FLOATS; ++i) {
		...
		for (const auto &sMD : vMDs)
		{
			...
			//f->addFilterInt(db::data_entries::FLAKETYPE, sFT);
			f->addFilterFloat(db::data_entries::FREQUENCY_GHZ, sFr);
			f->addFilterFloat(db::data_entries::TEMPERATURE_K, sT);
			f->addFilterFloat(db::data_entries::AEFF_UM, sAe);
			f->addFilterFloat(db::data_entries::MAX_DIMENSION_MM, sMD);
...
//				out << "\tMIN\tMAX\tMEDIAN\tMEAN\tSD\tSKEWNESS\tKURTOSIS\n";
	...
	//			out << data_entries::stringify<float>(i) << std::endl;
				out << "\t" << stats->floatStats(db::data_entries::S_MIN,i) << "\t"
					<< stats->floatStats(db::data_entries::S_MAX,i) << "\t"
					<< stats->floatStats(db::data_entries::MEDIAN,i) << "\t"
					<< stats->floatStats(db::data_entries::MEAN,i) << "\t"
					<< stats->floatStats(db::data_entries::SD,i);
			}
			out << std::endl;
			//stats->print(cerr);
		}
