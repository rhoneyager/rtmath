...
!   ANY DAMAGES THAT MAY RESULT FROM THE USE OF THE PROGRAM. 
      ...
      subroutine tmatrix_double() BIND(C, NAME='tmatrix_double_')
...
#include "common.defs"
     ...
     &     IT21(NPN6,NPN4,NPN4),IT22(NPN6,NPN4,NPN4)
      ...
      !DEC$ ATTRIBUTES DLLEXPORT::tmatrix_double
      !DEC$ ATTRIBUTES DLLEXPORT::/INPUTS/
      !DEC$ ATTRIBUTES DLLEXPORT::/INPUTSB/
      !DEC$ ATTRIBUTES DLLEXPORT::/OUTPUTS/
!      !DEC$ ATTRIBUTES DLLEXPORT::/TMAT/
...
!      IF(NP.EQ.-1.AND.EPS.GE.1D0) WRITE (6,7000) EPS
!      IF(NP.EQ.-1.AND.EPS.LT.1D0) WRITE (6,7001) EPS
!      IF(NP.GE.0) WRITE (6,7100) NP,EPS
!      IF(NP.EQ.-2.AND.EPS.GE.1D0) WRITE (6,7150) EPS
!      IF(NP.EQ.-2.AND.EPS.LT.1D0) WRITE (6,7151) EPS
...
!      WRITE (6,7200) DDELT
 ...
 7400 FORMAT('LAM=',F10.6,3X,'MRR=',D10.4,3X,'MRI=',D10.4)
      DDELT=0.1D0*DDELT
!      IF (DABS(RAT-1D0).LE.1D-6) WRITE (6,8003) AXI
!      IF (DABS(RAT-1D0).GT.1D-6) WRITE (6,8004) AXI
 ...
 8004 FORMAT('EQUAL-SURFACE-AREA-SPHERE RADIUS=',F8.4)
      ...
      INM1=MAX0(4,IXXX)
      IF (INM1.GE.NPN1) WRITE (6,7333) NPN1
      IF (INM1.GE.NPN1) QERROR=1
      IF (INM1.GE.NPN1) RETURN
 7333 FORMAT('CONVERGENCE IS NOT OBTAINED FOR NPN1=',I3, & 
     &       '.  EXECUTION TERMINATED')
      ...
      DO 50 NMA=INM1,NPN1
         ...
         MMAX=1
         NGAUSS=NMAX*NDGS
         IF (NGAUSS.GT.NPNG1) WRITE (6,7340) NGAUSS
         IF (NGAUSS.GT.NPNG1) QERROR=2
         IF (NGAUSS.GT.NPNG1) RETURN
...
!        PRINT 7334, NMAX,DSCA,DEXT
         ...
         IF (NMA.EQ.NPN1) WRITE (6,7333) NPN1
         IF (NMA.EQ.NPN1) QERROR=3
         IF (NMA.EQ.NPN1) RETURN      
   50 CONTINUE
   55 NNNGGG=NGAUSS+1
      MMAX=NMAX
!      IF (NGAUSS.EQ.NPNG1) WRITE (6,7336)
...
!         IF (NGAUS.EQ.NPNG1) WRITE (6,7336)
  150 CONTINUE
  155 CONTINUE
      QSCA=0D0
      QEXT=0D0
...
!        PRINT 7800,M,DABS(QXT),QSC,NMAX
 7800    FORMAT(' m=',I3,'  qxt=',D12.6,'  qsc=',D12.6, &
  ...
  220 CONTINUE
      WALB=-QSCA/QEXT
!      IF (WALB.GT.1D0+DDELT) WRITE (6,9111)
 9111 FORMAT ('WARNING: W IS GREATER THAN 1')

! The tmatrix is considered computed by this point. The values can be 
! returned. Any further code acts to compute the S and Z matrices
      ITIME=MCLOCK()
      TIME=DFLOAT(ITIME)/100D0
...
!  COMPUTATION OF THE AMPLITUDE AND PHASE MATRICES
      subroutine tmatrix_amplpha() BIND(C, NAME='tmatrix_amplpha_')
      use, intrinsic :: ISO_C_BINDING
      IMPLICIT REAL*8 (A-H,O-Z)
      INCLUDE 'ampld.par.f'
#include "common.defs"
      COMPLEX*16 S11,S12,S21,S22
!      COMMON /TMAT/ RT11,RT12,RT21,RT22,IT11,IT12,IT21,IT22
      !DEC$ ATTRIBUTES DLLEXPORT::tmatrix_amplpha
      !DEC$ ATTRIBUTES DLLEXPORT::/INPUTS/
      !DEC$ ATTRIBUTES DLLEXPORT::/INPUTSB/
      !DEC$ ATTRIBUTES DLLEXPORT::/INPUTSANG/
      !DEC$ ATTRIBUTES DLLEXPORT::/OUTPUTSAMP/
!      !DEC$ ATTRIBUTES DLLEXPORT::/TMAT/

!      PRINT *,"Calling ampl ", NMAX
!  AMPLITUDE MATRIX [Eqs. (2)-(4) of Ref. 6]
      DLAM=LAM
      CALL AMPL (NMAX,DLAM,THET0,THET,PHI0,PHI,ALPHA,BETA, &
     &           S11,S12,S21,S22)

!      PRINT *, NMAX, DLAM, THET0, THET, PHI0, PHI
!      PRINT *, ALPHA, BETA
...
! Pass back the phase and amplitude matrices to the common block used by the calling program
!      COMMON /OUTPUTS/ OS(8), OP(16)
...
!      IF (NMAX.GT.NPN1) PRINT 9000,NMAX,NPN1
      IF (NMAX.GT.NPN1) QERROR=23
      IF (NMAX.GT.NPN1) RETURN
...
!**********************************************************************
  ...
  100 CONTINUE
      ...
      PRINT 1100,N
 1100 FORMAT(' ***  POINTS AND WEIGHTS OF GAUSSIAN QUADRATURE', &
     & ' FORMULA OF ',I4,'-TH ORDER')
      DO 105 I=1,K
          ZZ=-Z(I)
