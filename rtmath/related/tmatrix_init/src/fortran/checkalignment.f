C This mini-module allows fortran code to throw some errors 
C back up to C++
      module throwers
      interface
...
C This program provides a function that checks the proper
C alignment of input parameters. It is intended for use in
C the testing portion of the library code. If this check fails,
C then running the code is pointless.
...
#include "common.defs"
      ...
      INTEGER (C_INT) :: x
C Now, check alignment based on default params in tmatrix.cpp
C Good alignment returns true, bad alignment returns false
