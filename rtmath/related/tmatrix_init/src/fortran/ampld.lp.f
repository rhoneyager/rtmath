...
C      !DEC$ ATTRIBUTES DLLEXPORT::/TMAT/
...
C      IF(NP.EQ.-1.AND.EPS.GE.1D0) WRITE (6,7000) EPS
C      IF(NP.EQ.-1.AND.EPS.LT.1D0) WRITE (6,7001) EPS
C      IF(NP.GE.0) WRITE (6,7100) NP,EPS
C      IF(NP.EQ.-2.AND.EPS.GE.1D0) WRITE (6,7150) EPS
C      IF(NP.EQ.-2.AND.EPS.LT.1D0) WRITE (6,7151) EPS
C      IF(NP.EQ.-3) PRINT 7160
C      WRITE (6,7400) LAM,MRR,MRI
C      WRITE (6,7200) DDELT
 ...
 7400 FORMAT('LAM=',F10.6,3X,'MRR=',D10.4,3X,'MRI=',D10.4)
      DDELT=0.1D0*DDELT
C      IF (DABS(RAT-1D0).LE.1D-6) WRITE (6,8003) AXI
C      IF (DABS(RAT-1D0).GT.1D-6) WRITE (6,8004) AXI
...
c        PRINT 7334, NMAX,DSCA,DEXT
   ...
   55 NNNGGG=NGAUSS+1
      MMAX=NMAX
C      IF (NGAUSS.EQ.NPNG1) WRITE (6,7336)
...
C         IF (NGAUS.EQ.NPNG1) WRITE (6,7336)
  150 CONTINUE
  155 CONTINUE
      QSCA=0D0
      QEXT=0D0
...
C        PRINT 7800,M,DABS(QXT),QSC,NMAX
 7800    FORMAT(' m=',I3,'  qxt=',D12.6,'  qsc=',D12.6,
  ...
  220 CONTINUE
      WALB=-QSCA/QEXT
C      IF (WALB.GT.1D0+DDELT) WRITE (6,9111)
 9111 FORMAT ('WARNING: W IS GREATER THAN 1')

C The tmatrix is considered computed by this point. The values can be 
C returned. Any further code acts to compute the S and Z matrices
      ITIME=MCLOCK()
...
c      PRINT *,"Calling ampl ", NMAX
C  AMPLITUDE MATRIX [Eqs. (2)-(4) of Ref. 6]
      DLAM=LAM
      CALL AMPL (NMAX,DLAM,THET0,THET,PHI0,PHI,ALPHA,BETA,
     &           S11,S12,S21,S22)

C      PRINT *, NMAX, DLAM, THET0, THET, PHI0, PHI
C      PRINT *, ALPHA, BETA
C      PRINT *, S11, " ", S12, " ", S21, " ", S22
      AERROR=0
...
C Pass back the phase and amplitude matrices to the common block used by the calling program
C      COMMON /OUTPUTS/ OS(8), OP(16)
