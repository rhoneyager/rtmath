...
namespace {
	...
	boost::shared_ptr<InterpMultilinear<1, double > > setupHanelA(hanelAmedium m)
	{
		std::lock_guard<std::mutex> lock(m_setup);
		static boost::shared_ptr<InterpMultilinear<1, double > > res_water_re, res_water_im, 
	...
	boost::shared_ptr<InterpMultilinear<1, double > > setupHanelB(hanelBmedium m)
	{
		std::lock_guard<std::mutex> lock(m_setup);
		static boost::shared_ptr<InterpMultilinear<1, double> > res_sand_o_re, res_sand_o_im, 
...
void rtmath::refract::implementations::mIceWarren(double f, double t, std::complex<double> &m)
{
	...
	static boost::shared_ptr<InterpMultilinear<2, double > > interp_ML_warren_re, interp_ML_warren_im;
	auto setupWarren = [&]()
	{
		std::lock_guard<std::mutex> lock(m_setup);
		if (setup) return;
		...
		interp_ML_warren_re = boost::shared_ptr<InterpMultilinear<2, double > >(new InterpMultilinear<2, double >
			(grid_iter_list.begin(), grid_sizes.begin(), vals_re.data(), vals_re.data() + num_elements));
		interp_ML_warren_im = boost::shared_ptr<InterpMultilinear<2, double > >(new InterpMultilinear<2, double >
	...
	m = std::complex<double>(interp_ML_warren_re->interp(args.begin()),
		interp_ML_warren_im->interp(args.begin()));
}
