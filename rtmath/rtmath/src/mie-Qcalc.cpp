...
namespace rtmath {
	namespace mie {
		...
		void Qcalc::calc(double sizep, double &Qext, double &Qsca, double &Qabs, double &Qbk, double &g) const
		{
			...
			double nc = sizep + 4.05*pow(sizep,1./3.) + 2.; // Candidate bounding formula
			...
			Qe *= 2.0/(sizep*sizep);
			Qs *= 2.0/(sizep*sizep);
			Qa = Qs-Qe; // Odd. Def. disagrees but this works. Fixes the negative sign issues.
			Qe = Qa + Qs; // So, a fix is given here.
