...
namespace rtmath
{
	namespace registry
	{
		...
		std::shared_ptr<rtmath::registry::IOhandler> write_file_type_multi
			...
			const T *obj, 
			const char* key,
			IOhandler::IOtype iotype); // = IOhandler::IOtype::TRUNCATE);
		...
		* \param U is the pobject multi-writer handler type
		...
		**/
			...
			const char* pluginid)
		{
			...
			res.io_multi_matches = std::bind(match_file_type_multi, _1, _2, _3, pluginid, extension);

			res.io_multi_processor = write_file_type_multi<T>;
			res.io_processor = std::bind(write_file_type_single<T>, _1, _2, IOhandler::IOtype::TRUNCATE);
		...
		void genAndRegisterIOregistry(
			...
			const char* pluginid)
		{
			auto res = genIOregistry<T>(extension, pluginid);
